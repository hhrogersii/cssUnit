...
(function ()
{
    ...
    */
    ...
    Output.prototype.show_results = function (tests, harness_status) {
        ...
        function fmt_tableheader(label) {
            var header = "<thead>";
            if (label) {
                header += "<tr class='test-label'><th colspan=" + (assertions ? 4 : 3) + ">" +
                    "<h3>" + label + "</h3></th></tr>";
            }
            header += "<tr><th>Result</th><th>Test Name</th>" +
        ...
        var html = "<h2>Results</h2><table id='results' " + (assertions ? "class='assertions'" : "" ) + ">";

        var grouping;
        for (var i = 0; i < tests.length; i++) {
            var t = tests[i];
            var g = t.properties.label;
            if (g !== grouping) {
                if (grouping) {
                    html += '</tbody>';
                }
                html += fmt_tableheader(g);
                grouping = g;
            }

            html += '<tr class="' +
                escape_html(status_class(status_text[t.status])) +
                '"><td>' +
                escape_html(status_text[t.status]) +
                "</td><td>" +
                escape_html(t.name.split(':')[1]) +
                "</td><td>" +
                (assertions ? escape_html(get_assertion(t)) + "</td><td>" : "") +
                escape_html(t.message ? t.message : " ") +
                (t.stack ? "<pre>" +
                 escape_html(t.stack) +
                 "</pre>": "") +
                "</td></tr>";
